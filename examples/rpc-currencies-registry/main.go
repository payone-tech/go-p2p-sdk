...
func main() {
	...
	request := proto.RpcRequest{
		Method: api.MethodRegistryCurrencies,
		Params: nil,
	}
	...
	var protoResponse *proto.CurrenciesResponse
	...
	fmt.Printf("count: %d\n", protoResponse.Count)
	for _, currency := range protoResponse.Currencies {
		fmt.Printf("uuid: %q, title: %q, code: %q\n",
			currency.UUID, currency.Title, currency.Code)
	}
}
